"""A parent dropped from top-K mid-loop receives no further samples."""
...
def test_dropped_parent_keeps_prior_samples_and_gets_no_new_ones(
    ...
    # Same setup as topk-shift test: A regresses out after round 1.
    by_parent: dict[str, list[float]] = {
        a.trial_id: [1e9],
        b.trial_id: [11e9, 11e9, 11e9],
        c.trial_id: [10e9, 10e9],
    }
    with patch.object(OptimizationLoop, "_evaluate", _refine_factory(by_parent)):
        created = loop.run_refinement(top_k=2, rounds=3)

    counts: dict[str, int] = {}
    for tr in created:
        counts[tr.parent_trial_id or ""] = counts.get(tr.parent_trial_id or "", 0) + 1
    # A had one round-1 sample and never returned to top-K.
    assert counts.get(a.trial_id, 0) == 1
    # B held top-K through every round.
    assert counts.get(b.trial_id, 0) == 3
    # C entered top-K in round 2 onward.
    assert counts.get(c.trial_id, 0) == 2
    # A's pre-existing refinement sample remains in _completed.
    a_samples = [
        tr
        for tr in loop._completed
        if tr.phase == "refinement" and tr.parent_trial_id == a.trial_id
    ]
